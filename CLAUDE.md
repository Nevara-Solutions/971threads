...
- **Site width — single source of truth:** the shared side gutter is the CSS variable **`--page-gutter`** (in `layout/theme.liquid`: `1.5rem` mobile, `4rem` ≥750px). The layout is **full-width** (no max-width cap); the header and every section's content use `padding-inline: var(--page-gutter)`, so the navbar + all sections share the same edges and align. Change `--page-gutter` once to shift the whole site. New sections must use `var(--page-gutter)` (don't reintroduce Dawn's `.page-width`, never hardcode the gutter). Full-bleed images (hero/split/countdown) stay `width:100%`; only their inner content is guttered.
